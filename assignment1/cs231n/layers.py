...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        bn_param[running_mean] =  momentum * running_mean + (1 - momentum) * np.mean(x)
        bn_param[running_var] = momentum * running_var + (1 - momentum) * np.var(x)

        # by definition of paper (x_hat)
        x_hat = (x - np.mean) / np.sqrt(np.var(x) + eps)
...
def svm_loss(x, y):
    ...
    margins[np.arange(num_train), y] = 0  # ignore the correct margin
