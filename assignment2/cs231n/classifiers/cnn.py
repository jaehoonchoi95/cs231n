...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        # After passing through the conv layer, the shape of the output is
        # (num_filters, c, h, w)  // by definition

        # after polling layer, the shape of the output is (num_filters, h_out, w_out),
        # we need to flatten it

        h_out = 1 + (h - 2) // 2
        w_out = 1 + (w- 2) // 2
        p_dim = num_filters * h_out * w_out
    ...
    def loss(self, X, y=None):
        ...
        dx3, dw3, db3 = affine_backward(dscore, cache3)
